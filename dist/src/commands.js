...
async function handleSetup(apiKey) {
    ...
    try {
        const data = await xblFetch(apiKey, "/account", { ttlMs: 0 });
...
async function handleFriends(apiKey) {
    ...
    return formatFriendsList(normalizeList(raw, "people"));
}
...
async function handleAchievements(apiKey) {
    ...
    return formatAchievements(normalizeList(raw, "titles"));
}
async function handleGamePass(apiKey, sub) {
    ...
    return formatGamePass(normalizeList(raw), target.label);
}
