...
const CONFIRM_HINT =
  "This is a state-changing action. Re-run with confirm=true once the user has explicitly approved it.";
...
export function registerWriteTools(api: any, apiKey: string) {
  api.registerTool(
    {
      ...
      ownerOnly: true,
      async execute(_id: string, { xuid, confirm }: { xuid: string; confirm: boolean }) {
        if (!confirm) return toolResult(`Not added — ${CONFIRM_HINT}`);
  ...
  api.registerTool(
    {
      ...
      ownerOnly: true,
      async execute(_id: string, { xuid, confirm }: { xuid: string; confirm: boolean }) {
        if (!confirm) return toolResult(`Not removed — ${CONFIRM_HINT}`);
  ...
  api.registerTool(
    {
      ...
      ownerOnly: true,
      async execute(_id: string, { xuid, message, confirm }: { xuid: string; message: string; confirm: boolean }) {
        if (!confirm) return toolResult(`Not sent — ${CONFIRM_HINT}`);
