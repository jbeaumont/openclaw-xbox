...
import { XuidParamSchema, EmptyParamSchema, Profile, TitleHistoryEntry } from "../types.js";
import { normalizeList, formatTitleHistory } from "../format.js";
import { toolResult } from "../result.js";

async function ownXuid(apiKey: string): Promise<string | undefined> {
  const data = await xblFetch<{ profileUsers: Profile[] }>(apiKey, "/account");
  return data.profileUsers?.[0]?.id;
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function registerTitleTools(api: any, apiKey: string) {
  api.registerTool(
    {
      ...
      async execute() {
        ...
        if (!xuid) return toolResult("Could not resolve your XUID from the account profile.");
        const raw = await xblFetch<unknown>(apiKey, `/player/titleHistory/${encodeURIComponent(xuid)}`);
        return toolResult(formatTitleHistory(normalizeList<TitleHistoryEntry>(raw, "titles")));
      },
    }
  );
