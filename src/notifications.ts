...
import type { Friend, MediaItem } from "./types.js";
...
export interface NotifyState {
  ...
  sentToday: number;
  remindedDay?: string;
  /** Undelivered alert texts, awaiting a session to piggyback on. */
  pending: string[];
}

export const DEFAULT_NOTIFY_STATE: NotifyState = {
  initialized: false,
  onlineXuids: [],
  clipIds: [],
  day: "",
...
const DISABLE_HINT =
  "Xbox alerts are on — turn them off with `openclaw config set plugins.entries.openclaw-xbox.config.notifications.enabled false`.";
...
export function resolveNotifyOptions(cfg: NotificationsConfig | undefined): {
  opts: NotifyOptions;
  intervalMs: number;
} {
  return {
 ...
 */
export function computeNotification(
  prev: NotifyState,
  friends: Friend[],
  clipIds: string[],
  opts: NotifyOptions,
  now: Date
): { text?: string; state: NotifyState } {
  ...
  if (!prev.initialized) return { state: baseState };
  ...
  if (opts.newClips) {
    ...
    if (newCount > 0) events.push(`🎬 ${newCount} new game clip${newCount === 1 ? "" : "s"} captured`);
  }

  // No real transition, or daily cap reached: advance state, surface nothing.
  if (events.length === 0 || sentToday >= opts.maxPerDay) return { state: baseState };
...
function clipId(c: any): string | undefined {
  return c?.gameClipId ?? c?.contentId ?? c?.id ?? undefined;
}

async function readState(file: string): Promise<NotifyState> {
  try {
...
async function writeState(file: string, state: NotifyState): Promise<void> {
  try {
...
/** Tracks the most recent session so alerts can piggyback on the user's next turn. */
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function createNotificationService(api: any, apiKey: string, cfg: NotificationsConfig | undefined, holder: SessionHolder) {
  const { opts, intervalMs } = resolveNotifyOptions(cfg);
  let timer: ReturnType<typeof setInterval> | undefined;
  let stateFile = "";

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  async function tick(ctx: any): Promise<void> {
    try {
      const state = await readState(stateFile);
      const friends = normalizeList<Friend>(await xblFetch(apiKey, "/friends", { ttlMs: 0 }), "people");
      let clipIds = state.clipIds;
      if (opts.newClips) {
        const raw = await xblFetch(apiKey, "/dvr/gameclips", { ttlMs: 0 });
        clipIds = normalizeList<MediaItem>(raw, "values", "gameClips")
          .map((c) => clipId(c))
