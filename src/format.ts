...
export function formatRecentPlayers(people: Friend[], limit = 30): string {
  if (people.length === 0) return "No recent players found.";
  ...
  for (const p of people.slice(0, limit)) {
    ...
    let line = `• ${tag}`;
    if (p.presenceText) line += ` — ${p.presenceText}`;
    lines.push(line);
  }
  if (people.length > limit) lines.push(`…and ${people.length - limit} more.`);
  return lines.join("\n");
}

/** Format an Xbox activity feed/history list (item shapes vary, so be defensive). */
export function formatActivity(items: unknown[], label: string, limit = 15): string {
  if (items.length === 0) return `No ${label.toLowerCase()} items found.`;
...
export function formatAlerts(items: unknown[], limit = 20): string {
